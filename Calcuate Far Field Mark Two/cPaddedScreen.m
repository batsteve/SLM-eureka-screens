...
%   Detailed explanation goes here
    ...
    switch options.paddingMethod
        case 'fully nested loops'
            processedScreen = ones(options.xFullSize, options.yFullSize)*options.exteriorTransmittance;
                ...
                for kk = 1:options.yInputSize
                    ...
                    for jjj = 0:(1-options.xBlowupFactor)
                        for kkk = 0:(1-options.yBlowupFactor)
                            processedScreen(xOffset+xDiff+jjj, yOffset+yDiff+kkk) = options.bevelTransmittance;
                        end
                    end 

                    for jjj = 1:options.xBlowupFactor
                        for kkk = 1:options.yBlowupFactor
                            processedScreen(xOffset+xDiff+jjj, yOffset+yDiff+kkk) = data.screen(jj, kk);
                        end
                    end     
                end
            end
        ...
        case 'block by block'
            ...
            processedScreen(XX, YY) = options.bevelTransmittance;
            ...
            for jj = 0:(options.xRawSize-1)
                for kk = 0:(options.yRawSize-1)
                    ...
                    processedScreen(XXX, YYY) = data.screen(jj+1, kk+1);
                end
            end
        ...
        case 'travelling raw';
            ...
            for jj = 1:(options.xBlowupFactor)
                for kk = 1:(options.yBlowupFactor)
                    processedScreen(XXX+jj, YYY+kk) = data.screen(:, :);
                end
            end
